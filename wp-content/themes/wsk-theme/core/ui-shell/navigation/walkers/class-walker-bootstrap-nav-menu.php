<?php
/**
 * Walker - Bootstrap Nav Menu
 *
 * A custom WordPress Walker class to implement a three level Bootstrap 5 navigation menu.
 *
...
/**
 * Walker_Bootstrap_Nav_Menu class.
 *
...
class Walker_Bootstrap_Nav_Menu extends Walker_Nav_Menu {
	...
	function start_lvl( &$output, $depth = 0, $args = null ) {
		...
		$output .= "\n$indent<ul class=\"dropdown-menu$submenu dropdown-menu-end dropdown-menu-xxxl-start " . esc_attr( implode( ' ', $dropdown_menu_class ) ) . " dropdown-menu--depth-$depth\">\n";
	}
	...
	function start_el( &$output, $item, $depth = 0, $args = null, $id = 0 ) {
		...
		if ( $args->walker->has_children ) {
			$item_output .= '<span class="dropdown-toggle__icon"></span>';
		}
		$item_output .= $args->link_before . apply_filters( 'the_title', $item->title, $item->ID ) . $args->link_after;
		if ( $args->walker->has_children ) {
			$item_output .= wskt_get_icon(
				'chevron-down',
				'wsk',
				array(
					'width'  => 14,
					'height' => 8,
					'class'  => 'dropdown-chevron__icon',
				)
			);
		}
		$item_output .= '</a>';
